...
\begin{theorem}
  Existe una única función $\phi_{Shapley}$ \index{$\phi_{Shapley}$}
...
conjunto de entrenamiento: \index{$\phi_{loo}$}
\[
...
anterior a: \index{Beta} \index{$\Gamma$} \index{$w_{\alpha, \beta}$}
\begin{equation*}
  \begin{split}
  \label{eq:pesoBeta}
...
Banzhaf, visto en la sección \ref{ec:banzhafFormula} en el
